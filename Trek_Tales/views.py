        ...
        try:
            ...
            if(instance.Payment_Status):
                ...
                refund = ReFund(Name=instance.Name, ReFund_For=instance.slotFor, gender=instance.gender, TripId=instance.TripId, email=instance.email, Phone_no1=instance.Phone_no1, address=instance.address, amount=instance.amount, razorpay_payment_id=instance.razorpay_payment_id,
                                razorpay_order_id=instance.razorpay_order_id, razorpay_signature=instance.razorpay_signature, Payment_Status=instance.Payment_Status, refund_Amount=RefundAmout, charge_Day=cancelation_charge_days, charge_percenrate=charges)
                refund.save()
...
def ShowBookingsOfId(request, id):
    tours = Updates.objects.order_by('-tour_on_date')
    BookedSlots = BookSlot.objects.filter(slotFor=id)
    refund = ReFund.objects.filter(ReFund_For=id).order_by('-charge_Day')
...
def SendNewsletter(request):
    ...
    fs = FileSystemStorage(location='templates').listdir('newsletter')
    templates = list()
    for i in fs:
        for j in i:
            try:
